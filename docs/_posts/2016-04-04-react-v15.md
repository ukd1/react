...
- #### `document.createElement` is in and `data-reactid` is out
    ...
    ([@spicyj](https://github.com/spicyj) in [#5205](https://github.com/facebook/react/pull/5205))
...
We also made a bugfix to `React.cloneElement(element, props)` that is technically a breaking change. If some of the passed `props` were `undefined`, `cloneElement()` in React 0.14 returned an element with `undefined` values for those props. In React 15, it is changed to be consistent with `React.createElement()`, and any `undefined` props are resolved to the corresponding component’s `defaultProps`. In a codebase with more than 20,000 React components, we only a single place affected by this so we feel comfortable releasing it without keeping the old behavior for another release cycle. In most cases, this should be the desired behavior so you likely won’t need to change anything. ([@truongduy134](https://github.com/truongduy134) in [#5997](https://github.com/facebook/react/pull/5997))

There is one additional breaking change that is unlikely to affect applications but may affect some third-party tools. We are working on [revamping `ReactPerf` instrumentation](https://github.com/facebook/react/pull/6046) to be more accurate and provide more detailed information. We plan to introduce this change in a minor 15.x release, and since the internal performance measurement format will change, the return value of `ReactPerf.getLastMeasurements()` is now considered an opaque data structure. We will update the documentation if its format becomes an officially supported part of public API again. ([@gaearon](https://github.com/gaearon) in [#6268](https://github.com/facebook/react/pull/6286))
...
- Deprecated APIs are removed from `React.addons`, specifically `batchedUpdates` and `cloneWithProps`. ([@jimfb](https://github.com/jimfb) in [#5859](https://github.com/facebook/react/pull/5859), [@zpao](https://github.com/zpao) in [#6016](https://github.com/facebook/react/pull/6016))
- Deprecated APIs are removed from component instances, specifically `setProps`, `replaceProps`, and `getDOMNode`. ([@jimfb](https://github.com/jimfb) in [#5570](https://github.com/facebook/react/pull/5570) and [#5833](https://github.com/facebook/react/pull/5833))
- Deprecated CommonJS entry point `react/addons` is removed in favor of the separate `react-addons-*` packages. This does not affect the `react-with-addons.js` UMD build. ([@gaearon](https://github.com/gaearon) in [#6285](https://github.com/facebook/react/pull/6285))
...
- `LinkedStateMixin` and `valueLink` are now deprecated due to very low popularity. If you need this, you can use a wrapper component that implements the same behavior: [react-linked-input](https://www.npmjs.com/package/react-linked-input). ([@jimfb](https://github.com/jimfb) in [#5032](https://github.com/facebook/react/pull/5032), [#5718](https://github.com/facebook/react/pull/5718), and [#6127](https://github.com/facebook/react/pull/6127))
...
- `ReactPerf.printDOM()` has been renamed to `ReactPerf.printOperations()`, and `ReactPerf.getMeasurementsSummaryMap()` was renamed to `ReactPerf.getWasted()`. ([@gaearon](https://github.com/gaearon) in [#6287](https://github.com/facebook/react/pull/6287))
...
- React DOM now warns if you specify `onFocusIn` or `onFocusOut` attributes which correspond to `onFocus` in React. ([@jontewks](https://github.com/jontewks) in [#6296](https://github.com/facebook/react/pull/6296))
...
- Fixed multiple small memory leaks. ([@spicyj](https://github.com/spicyj) in [#4983](https://github.com/facebook/react/pull/4983), [@victor-homyakov](https://github.com/victor-homyakov) in [#6309](https://github.com/facebook/react/pull/6309))
...
- `React.cloneElement()` attempts to preserve the existing `key` when used inside `React.Children.map()`. ([@ianobermiller](https://github.com/ianobermiller) in [#5892](https://github.com/facebook/react/pull/5892))
...
- React DOM does not throw when removing a `<select>` while executing its `onChange` handler. ([sambev](https://github.com/sambev) in [#6028](https://github.com/facebook/react/pull/6028))
...
- React now uses `loose-envify` instead of `envify` so it installs less transitive dependencies. ([@qerub](https://github.com/qerub) in [#6306](https://github.com/facebook/react/pull/6303))
