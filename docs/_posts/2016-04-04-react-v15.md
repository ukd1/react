...
- #### `document.createElement` is in and `data-reactid` is out
    ...
    Note that `data-reactid` is still present in the markup generated when rendering on the server although there [may be ways](https://github.com/facebook/react/issues/5869) to remove it completely in the future releases. Please feel free to explore this and send a pull request if you get it working.

    If you used `data-reactid` to detect websites using React, you can now look for `data-reactroot` instead. Unlike `data-reactid`, `data-reactroot` will only be set on the root DOM node. Still, these attributes are considered to be implementation details of React, and we suggest you to avoid relying on their presence.
